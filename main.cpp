...
public:
    ...
    void run()
    {
        ...
        SocketAddress address=SocketAddress("192.168.73.11",8080);
        ...
        while (sample_num<100){
            ...
            pvalue = BSP_PSENSOR_ReadPressure();
            ...
            float ax =pAccDataXYZ [0], ay = pAccDataXYZ [1],az = pAccDataXYZ [2];
            float mx =pMagDataXYZ [0], my = pMagDataXYZ [1],mz = pMagDataXYZ [2];
            float gx =pGyroDataXYZ [0], gy = pGyroDataXYZ [1],gz = pGyroDataXYZ [2];

            int alen = sprintf(acc ,"{\"ax \":% f,\"ay \":% f,\"az \":% f}|",(float)(( int )(ax*10000))/10000,(float)((int )(ay*10000))/10000, (float)(( int )(az*10000))/10000);
            int aresponse = _socket.send(acc,alen);
            
            int mlen = sprintf(mag ,"{\"mx \":% f,\"my \":% f,\"mz \":% f}|",(float)(( int )(mx*10000))/10000,(float)((int )(my*10000))/10000, (float)(( int )(mz*10000))/10000);
            int mresponse = _socket.send(mag,mlen);

            int glen = sprintf(gyro ,"{\"gx \":% f,\"gy \":% f,\"gz \":% f}|",(float)(( int )(gx*10000))/10000,(float)((int )(gy*10000))/10000, (float)(( int )(gz*10000))/10000);
            int gresponse = _socket.send(gyro,glen);

            int tlen = sprintf(temp,"temperature=%f|",tvalue);
            int tresponse = _socket.send(temp,tlen);

            int hlen = sprintf(humid ,"humidity=%f|",hvalue);
            int hresponse = _socket.send(humid,hlen);

            int plen = sprintf(pre,"pressure=%f",pvalue);
            int presponse = _socket.send(pre,plen);

            if (0 >= aresponse){
                printf("Error seding : %d n", aresponse);}
